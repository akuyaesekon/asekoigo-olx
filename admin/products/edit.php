...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
        ...
        if (!in_array($fileType, $allowedTypes)) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $targetPath)) {
                ...
                if ($image && $image != 'default.jpg' && file_exists($uploadDir . $image)) {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("
            UPDATE products 
            SET name = ?, description = ?, price = ?, image = ?, stock = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        ");
        
        if ($stmt->execute([$name, $description, $price, $image, $stock, $productId])) {
            ...
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control <?php echo isset($errors['description']) ? 'is-invalid' : ''; ?>" 
                              id="description" name="description" rows="5" required><?php echo htmlspecialchars($product['description']); ?></textarea>
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" class="form-control <?php echo isset($errors['price']) ? 'is-invalid' : ''; ?>" 
                           id="price" name="price" value="<?php echo htmlspecialchars($product['price']); ?>" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control <?php echo isset($errors['stock']) ? 'is-invalid' : ''; ?>" 
                           id="stock" name="stock" value="<?php echo htmlspecialchars($product['stock']); ?>" required>
                ...
                <div class="mb-3">
                    ...
                    <div class="form-text">Current image: 
                        <?php if ($product['image']): ?>
                            <img src="<?php echo APP_URL; ?>/assets/images/products/<?php echo $product['image']; ?>" 
                                 alt="Current product image" width="50" class="ms-2">
                        <?php else: ?>
                            None
                        <?php endif; ?>
                    </div>
